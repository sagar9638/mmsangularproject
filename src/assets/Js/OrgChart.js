var OrgChart = function(t, e) {
    ...
    if (this._event_id = OrgChart._guid(), !this.config.searchFields.length && this.config.nodeBinding)
        for (var o in this.config.nodeBinding) - 1 == o.indexOf("img") && "function" != typeof this.config.nodeBinding[o] && this.config.searchFields.push(this.config.nodeBinding[o]);
...
OrgChart.prototype.load = function(t) {
    ...
    this.server.read(t, this.width(), this.height(), e, C, y, function(t) {
        ...
        if (OrgChart.events.publish("render", [O, e]), r = e.content, r += O.ui.lonely(O.config), C !== OrgChart.action.exporting) {
            ...
            C === OrgChart.action.centerNode ? OrgChart.anim(O.getSvg(), {
                viewbox: a
            }, {
                viewbox: O.response.viewBox
            }, O.config.anim.duration, O.config.anim.func, function() {
            ...
            a = function(t) {
                if (_) {
                    ...
                    if (a.left || a.right || a.up || a.down) {
                        ...
                        if (b.stopMove(), w.classList && (w.classList.add("cursor-grab"), w.classList.remove("cursor-move"), w.classList.remove("cursor-nodrop"), w.classList.remove("cursor-copy")), L(O, v), v = O = null, b.config.enableDragDrop)
                            for (; null != r && r != w;) {
...
OrgChart.prototype._mouseDownHandler = function(t, e, r) {
    ...
    if (OrgChart.server._initNode(e, r, s, i, c, h, d, g, p, f, u), e.isAssistant && (n[e.pid] || (n[e.pid] = []), n[e.pid].push(e.id)), 0 < e.subLevels && o.push(e.id), !e.isAssistant && 0 == e.childrenIds.length && e.parent && !l[e.pid]) {
        for (var m = [], C = 0, y = 0, x = 0; x < e.parent.children.length; x++) {
            var O = e.parent.children[x]; - 1 == O.tags.indexOf("partner") && -1 == O.tags.indexOf("assistant") && 0 == O.childrenIds.length ? m.push(O.id) : -1 != O.tags.indexOf("assistant") ? C++ : -1 != O.tags.indexOf("partner") && y++
        }
    ...
    for (var o = a.nodes, l = {}, s = [], h = [], d = 0; d < o.length; d++) {
        ...
        null != g.ppid && (f.ppid = g.ppid), g.stpid && (f.stpid = g.stpid), null != a.orderBy && (f.order = OrgChart.server._getOrderFieldValue(g, a.orderBy)), l[g.id] = f, s.push(g.id)
    }
    ...
    for (d = 0; d < E.length; d++) {
        ...
        for (w = N.length - 1; 0 <= w; w--)
            if (T[N[w]])
                for (var j = 0; j < T[N[w]].length; j++) f.children.push(l[T[N[w]][j]]), P++, -1 == H.indexOf(N[w]) && H.push(N[w]);
        ...
        for (w = 0; w < M.length; w++)
            if (T[M[w]])
                for (j = 0; j < T[M[w]].length; j++) f.children.push(l[T[M[w]][j]]), F++, -1 == z.indexOf(M[w]) && z.push(M[w]);
    ...
    for (d = 0; d < I.length; d++) {
        if ((et = t[(f = l[I[d]]).lcn ? f.lcn : "base"]).layout == OrgChart.normal || !A[f.pid])
            for (w = 0; w < f.subLevels; w++) {
                var U = new OrgChart.node(f.id + "_sub_level_index_" + w, f.pid, [], "subLevel");
                if (OrgChart.server._initDinamicNode(U, f.lcn), y = f.parent) {
                    var D = y.children.indexOf(f); - 1 < D && (y.children.splice(D, 1), y.children.splice(D, 0, U)), U.children.push(f), U.parent = y, f.parent = U, l[U.id] = U
                }
            }
    }
    for (var Y in S) {
        (y = l[Y]).hasAssistants = !0;
        U = new OrgChart.node(y.id + "_split_0", y.id, ["assistant"], "split");
        OrgChart.server._initDinamicNode(U, y.lcn, !0), l[U.id] = U;
        var X = [];
        for (w = y.children.length - 1; 0 <= w; w--) {
            (R = y.children[w]).isAssistant ? (R.parent = null, y.children.splice(w, 1), X.splice(0, 0, R.id)) : R.isPartner || ((R.parent = U).children.unshift(R), y.children.splice(w, 1))
        }
        if (X.length % 2) {
        ...
        for (var Y in A) {
            ...
            if ((et = t[(y = l[Y]).lcn ? y.lcn : "base"]).layout == OrgChart.mixed || et.layout == OrgChart.tree || et.layout == OrgChart.treeRightOffset || et.layout == OrgChart.treeLeftOffset) {
                ...
                if (OrgChart.events.publish("layout", [rt]), rt.layout == OrgChart.mixed) {
                    ...
                    U = new OrgChart.node(y.id + "_split_0", y.id, [], "split");
                    OrgChart.server._initDinamicNode(U, y.lcn), (l[U.id] = U).layout = OrgChart.tree;
                    var nt = [];
    ...
    link: function(t, e, r, i, a) {
        ...
        for (f = 0; f < t.children.length; f++) {
            ...
            if (p && -1 != p.ids.indexOf(m.id)) switch (o.orientation) {
                ...
                case OrgChart.orientation.top:
                case OrgChart.orientation.top_left:
                    if (1 == m.isPartner) y = OrgChart.ui._linkLeftToRight(t, m, l, d);
                    else if (2 == m.isPartner) y = OrgChart.ui._linkRightToLeft(t, m, l, d);
                    else {
                ...
                case OrgChart.orientation.right:
                case OrgChart.orientation.right_top:
                    if (1 == m.isPartner) y = OrgChart.ui._linkTopToBottom(t, m, l, d);
                    else if (2 == m.isPartner) y = OrgChart.ui._linkBottomToTop(t, m, l, d);
                    else {
            ...
            if (-1 != x.indexOf("{rounded}"))
                if (y.xa == y.xb && y.xa == y.xc && y.xa == y.xd || y.ya == y.yb && y.ya == y.yc && y.ya == y.yd) x = x.replace("{rounded}", "M" + y.xa + "," + y.ya + " L" + y.xd + "," + y.yd);
                else {
    ...
    if (n.length) {
        ...
        for (var d = 0; d < n.length; d++)
            if (l += n[d].length, n[d].start < s && (s = n[d].start), 1 <= d && n[d - 1].start > n[d].start) {
                h = 0;
                break
            }
    }
    ...
    if (!OrgChart.isNullOrEmpty(t))
        for (var r = 0; r < e.length; r++) {
            ...
            if (!OrgChart.isNullOrEmpty(i))
                for (var a = 0; - 1 < (a = t.indexOf(i, a));) l.push({
                    length: i.length,
                    start: a
                }), a += i.length
        }
